...
class Maze:
    def __init__(self, width=15, height=17):  # ukuran maze diperkecil
        self.width = width
        self.height = height
    ...
    def generate_maze(self):
        # Buat frame dinding luar (pasti)
        for y in range(self.height):
            for x in range(self.width):
                if x == 0 or x == self.width-1 or y == 0 or y == self.height-1:
                    self.grid[y][x] = 1
                else:
                    self.grid[y][x] = 0  # awal semua jalan

        mid_x = self.width // 2
        mid_y = self.height // 2

        # Kotak 4x4 di tengah (bisa disesuaikan)
        for y in range(mid_y-2, mid_y+2):
            for x in range(mid_x-2, mid_x+2):
                self.grid[y][x] = 1

        # Tambah dinding random, lebih sedikit agar pas ukuran kecil
        for _ in range(30):
        ...
        # Letakkan pellet di tempat kosong
        for y in range(self.height):
            for x in range(self.width):
                if self.grid[y][x] == 0:
                    self.grid[y][x] = 2
    ...
    def draw(self, screen):
        tile_size = 16  # lebih kecil dari 24
